...
class CLSTM(nn.Module):
    ...
    def __init__(self, shape, input_chans, filter_size, num_features,num_layers):
        ...
        cell_list.append(CLSTM_cell(self.shape, self.input_chans, 
                   self.filter_size, self.num_features).cuda())#the first
        #one has a different number of input channels
        
        for idcell in range(1,self.num_layers):
            cell_list.append(CLSTM_cell(self.shape, self.num_features, 
                   self.filter_size, self.num_features).cuda())
...
# for p in params:
    ...
    print('out[1] shape',out[1].size())
    print('len hidden ', len(out[0]))
# print('out[0][0] shape', out[0][0])
